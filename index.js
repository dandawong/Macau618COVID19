...
function fixSpecialCase(text) {
  ...
  if (text.includes('238-618')) {
    text = text.replace('2022/06/245', '2022/06/25')
  }

  if (text.includes('216-618')) {
    text = text.replace(/6月22日起於高冠街16號萬宜\s/gm, '6月22日起於高冠街16號萬宜')
  }
...
while ((dirent = dir.readSync()) !== null) {
  ...
  pdf(dataBuffer).then(function (data) {
    ...
    var reTime = new RegExp(/凌晨|整天|上午\s–\s中午|^(\d+:\d+|全天|中午\-晚上|下午\-晚上|早上|--|\d+:\d+|中午|上午|下午|晚上|全日|早上|\d+:\d+\-\d+:\d+|\d+:\d+\-|\d*:\d*~\d*:\d*|早上至晚上|\d+:\d+至\d+:\d+)\s+/)
    ...
    textList.forEach(function (text, index) {
      ...
      if (traceStart) {
        ...
        if (content.trim().length > 0 && currentDate != null) {
          // if (currentTime == null) console.log(text, fileName, textList)
          let trace = {}
